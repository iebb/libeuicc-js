...
int es10b_prepare_download(struct euicc_ctx *ctx, const char *confirmationCode)
{
    ...
    {
        printf("es10b_prepare_download: already prepare download response\n");
        return -1;
    }

    if (ctx->http._internal.prepare_download_param == NULL)
    {
        printf("es10b_prepare_download: no prepare download param\n");
        return -1;
    }
    ...
    if (fret < 0)
    {
        printf("es10b_prepare_download: prepare download failed\n");
