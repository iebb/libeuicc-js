...
static int es9p_trans_ex(struct euicc_ctx *ctx, const char *url, const char *url_postfix, uint32_t *rcode, char **str_rx, const char *str_tx)
{
    ...
    if (http_transmit(full_url, &rcode_mearged, &rbuf, &rlen, (const uint8_t *)str_tx, strlen(str_tx)) < 0)
    {
        goto err;
    }
    if (getenv("LIBEUICC_DEBUG_HTTP"))
...
int es9p_initiate_authentication_r(struct euicc_ctx *ctx, char **transaction_id, struct es10b_authenticate_server_param *resp, const char *server_address, const char *b64_euicc_challenge, const char *b64_euicc_info_1)
{
    ...
    const char *idata[] = {ctx->http._internal.server_address, b64_euicc_challenge, b64_euicc_info_1, NULL};
...
int es9p_get_bound_profile_package_r(struct euicc_ctx *ctx, char **b64_bound_profile_package, const char *server_address, const char *transaction_id, const char *b64_prepare_download_response)
{
    ...
    if (es9p_trans_json(ctx, server_address, "/gsma/rsp2/es9plus/getBoundProfilePackage", ikey, idata, okey, oobj, optr))
    {
        return -1;
    }
...
int es9p_initiate_authentication(struct euicc_ctx *ctx)
{
    ...
    fret = es9p_initiate_authentication_r(ctx, &ctx->http._internal.transaction_id_http, ctx->http._internal.authenticate_server_param, ctx->http._internal.server_address, ctx->http._internal.b64_euicc_challenge, ctx->http._internal.b64_euicc_info_1);
    if (fret < 0)
    {
...
int es9p_get_bound_profile_package(struct euicc_ctx *ctx)
{
    ...
    fret = es9p_get_bound_profile_package_r(ctx, &ctx->http._internal.b64_bound_profile_package, ctx->http._internal.server_address, ctx->http._internal.transaction_id_http, ctx->http._internal.b64_prepare_download_response);
    if (fret < 0)
    {
...
int es9p_authenticate_client(struct euicc_ctx *ctx)
{
    ...
    fret = es9p_authenticate_client_r(ctx, ctx->http._internal.prepare_download_param, ctx->http._internal.server_address, ctx->http._internal.transaction_id_http, ctx->http._internal.b64_authenticate_server_response);
    if (fret < 0)
    {
...
int es9p_cancel_session(struct euicc_ctx *ctx)
{
    ...
    fret = es9p_cancel_session_r(ctx, ctx->http._internal.server_address, ctx->http._internal.transaction_id_http, ctx->http._internal.b64_cancel_session_response);
    if (fret < 0)
    {
        return fret;
    }
...
int es11_authenticate_client(struct euicc_ctx *ctx, char ***smdp_list)
{
    ...
    fret = es11_authenticate_client_r(ctx, smdp_list, ctx->http._internal.server_address, ctx->http._internal.transaction_id_http, ctx->http._internal.b64_authenticate_server_response);
    if (fret < 0)
    {
        return fret;
    }
...
int es9p_handle_notification(struct euicc_ctx *ctx, const char *b64_PendingNotification)
{
    ...
    return es9p_trans_json(ctx, ctx->http._internal.server_address, "/gsma/rsp2/es9plus/handleNotification", ikey, idata, NULL, NULL, NULL);
}
